defmodule WarehouseGitpod.DeliveratorPoolTest do
  use ExUnit.Case
  alias WarehouseGitpod.{DeliveratorPool}
  doctest DeliveratorPool
  ...
  describe "#available_deliverator" do
    ...
    test "fetches idle deliverator" do
      {:ok, pid} = DeliveratorPool.available_deliverator()
      {:ok, idle_deliverator} = DeliveratorPool.available_deliverator()
      state = :sys.get_state(DeliveratorPool)

      assert is_pid(idle_deliverator)
      assert pid == idle_deliverator
      assert Enum.count(state.deliverators) > 0
    end

    test "resturns error when maxed out" do
      state = :sys.get_state(DeliveratorPool)
      number_to_add = DeliveratorPool.max - Enum.count(state.deliverators)
      ...
      assert {:error, "deliverator pool maxed out"} = response
    end
  end
